...
/*
		// 2015.03.27 kobake 暫定対処
		if(preg_match('/^Invalid process/', $exception->getMessage())){
			print("not found.<span style='display:none;'>caseAA</span>");
			exit;
		}
		if(preg_match('/^Controller class .+Controller could not be found./', $exception->getMessage())){
			print("not found.<span style='display:none;'>caseA</span>");
			exit;
		}
		if(preg_match('/^Action .+ could not be found./', $exception->getMessage())){
			print("not found.<span style='display:none;'>caseB</span>");
			exit;
		}
		if(preg_match('/^Invalid manual/', $exception->getMessage())){
			print("not found.<span style='display:none;'>caseC</span>");
			exit;
		}
		print("---- Error information ----<br/>\n");
		print("File: " . $exception->getFile() . ":" . $exception->getLine() . "<br/>\n");
		print("Message: " . $exception->getMessage() . "<br/>\n");
		print("---- Stack trace ----<br/>\n");
		$a = $exception->getTrace();
		foreach($a as $index => $line){
			print("#{$index} {$line['file']}({$line['line']}): {$line['class']}{$line['type']}{$line['function']}(");
			print(join(', ', $line['args']));
			// print(json_encode($line['args'], JSON_UNESCAPED_UNICODE));
			print(")<br/>\n");
		}
		// var_dump($a);
		print("---- Exception toString ----<br/>\n");
		print(str_replace("\n", "<br/>\n", $exception) . "<br/>\n");
		print("---- Exception var_dump ----<br/>\n");
		var_dump($exception);
		exit;
*/
